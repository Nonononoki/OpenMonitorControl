...
void MainWindow::initMonitors(Parsed_Cmd * parsed_cmd) {
    ...
    for (int ndx = 0; ndx < _dlist->ct; ndx++) {
        ...
        if(ndx == 0) {
        QObject::connect(baseModel,   &FeatureBaseModel::signalCapabilitiesLoaded,
                         this,        &MainWindow::firstMonitorCapabilitiesLoaded);
        }
...
void MainWindow::firstMonitorCapabilitiesLoaded() {
    if (_monitors.size() > 0)
